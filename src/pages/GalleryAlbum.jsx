...
export default function GalleryAlbum() {
  ...
  const GalleryYangDitampilkan = gallery.filter((item) => item.id == id);
  //   console.log(GalleryYangDitampilkan);
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
            <div className="col-span-2">
              {GalleryYangDitampilkan.map((item) => (
                <div key={item.id}>
                  <h3 className="mt-2 text-2xl capitalize  font-bold text-gray-800 dark:text-gray-100">
                    {item.judul_album}
                  </h3>
                  ...
                  <a
                    href={`${getImageUrlGallery(item.gambar)}`}
                    target="_blank"
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-4">
                    {galleryDetail.map((detail) => (
                      <div key={detail.id}>
                        <a
                          href={`${getImageUrlGallery(detail.gambar)}`}
                          target="_blank"
                          rel="noopener noreferrer"
                        >
                          <img
